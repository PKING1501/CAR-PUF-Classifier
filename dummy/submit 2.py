...
################################
	...
	# Calculate the model size in KB
	model_size_kb = sys.getsizeof(model) / 1024
	return w, b, model_size_kb
...
for t in range( n_trials ):
	tic = tm.perf_counter()
	w, b, model_size_kb = my_fit( Z_trn[:, :-1], Z_trn[:,-1] )
	toc = tm.perf_counter()
	t_train += toc - tic

	d_size += model_size_kb
...
print("Model Size (KB):", d_size)
print("Training Time:", t_train)
print("Mapping Time:", t_map)
